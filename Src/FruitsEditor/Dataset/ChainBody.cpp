	...
	: Body(world, scale)
{
	...
	for (size_t i = 0, n = fixturesData.size(); i < n; ++i)
	{
		...
		{
			transList[j].x = fixtureData->vertices[j].x * scale;
			transList[j].y = fixtureData->vertices[j].y * scale;
		}
...
ChainBody::ChainBody(b2World* world, const std::vector<f2Vec2>& vertices, bool isLoop, float scale/* = 1.0f*/)
	: Body(world, scale)
{
	b2BodyDef bd;
	bd.type = b2_staticBody;
	...
	{
		transList[i].x = vertices[i].x * scale;
		transList[i].y = vertices[i].y * scale;
	}
...
void ChainBody::getAABB(f2AABB& aabb, const f2Vec2& position, float angle) const
{
	aabb.makeInfinite();
