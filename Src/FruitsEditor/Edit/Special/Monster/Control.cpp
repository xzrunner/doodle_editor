...
Control::Control()
{
	...
	GroupDialog::setSelection(&m_model->getLayersMgr().getSelection());
	m_preview = new Preview(dynamic_cast<Blackboard*>(m_view->getBlackboard()));
}
...
void Control::handleUIEvent()
{
	while (m_view->hasNextEvent())
	{
		...
		case Blackboard::e_resizeBg:
			m_model->handleEvent(event);
			break;
		case Blackboard::e_Play:
			{
				Layer* layer = m_model->getLayersMgr().getLayer(Layer::e_middle);
				m_preview->load(layer);
				m_preview->start();
			}
			break;
		}
