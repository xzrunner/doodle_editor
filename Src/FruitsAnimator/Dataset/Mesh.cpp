...
void Mesh::insertNode(const f2Vec2& pos)
{
	if (!m_dt)
	{
		std::vector<f2Vec2> bound;
		bound.push_back(f2Vec2(-m_halfWidth, -m_halfHeight));
		bound.push_back(f2Vec2( m_halfWidth, -m_halfHeight));
		bound.push_back(f2Vec2( m_halfWidth,  m_halfHeight));
		bound.push_back(f2Vec2(-m_halfWidth,  m_halfHeight));
...
void Mesh::deleteNode(const f2Vec2& pos)
{
	std::vector<f2Vec2> deletePos;
...
void Mesh::tween(const Mesh& start, const Mesh& end, float process)
{
	...
	for (size_t i = 0, n = sTris.size(); i < n; ++i)
	{
		...
		for (size_t j = 0; j < 3; ++j)
		{
			f2Vec2 offset = (eNodes[j]->projCoords - sNodes[j]->projCoords) * process;
...
void Mesh::loadTrisFromDT()
{
	...
	for (size_t i = 0, n = trisSrc.size(); i < n; ++i)
	{
		...
		std::vector<f2Vec2> triBound;
		src->getNodesPos(triBound);

		f2Vec2 p = f2Math::getTriGravityCenter(triBound[0], triBound[1], triBound[2]);
		if (f2Math::isPointInArea(p, bound))
		{
...
MeshNode::MeshNode(const f2Vec2& pos, float hWidth, float hHeight)
{
...
void MeshNode::initCoords(const f2Vec2& pos, float hWidth, float hHeight)
{
	projCoords = pos;
