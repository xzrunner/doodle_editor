...
void KeyFrame::loadFromTextFile(std::ifstream& fin, const LibraryPanel& library)
{
	clear();
	...
	for (size_t i = 0; i < size; ++i)
	{
		int type;
		fin >> type;
		Sprite* sprite = SpriteFactory::createSprite(SpriteFactory::SPRITE_TYPE(type));
		sprite->loadFromTextFile(fin, library);
		m_sprites.push_back(sprite);
	}
}

void KeyFrame::storeToTextFile(std::ofstream& fout, const LibraryPanel& library) const
{
	fout << m_bClassicTween << " ";
	fout << "sprite " << m_sprites.size() << '\n';
	for (size_t i = 0, n = m_sprites.size(); i < n; ++i)
		m_sprites[i]->storeToTextFile(fout, library);
}
...
bool KeyFrame::removeSprite(f2Sprite* sprite)
{
