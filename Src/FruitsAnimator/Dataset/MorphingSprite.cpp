...
MorphingSprite::MorphingSprite(Symbol* symbol, const f2Vec2& pos, bool isCopyMesh)
{
	m_pos = pos;
	m_symbol = symbol;
	initBounding(symbol);
...
void MorphingSprite::loadFromTextFile(std::ifstream& fin, const LibraryPanel& library)
{
	clear();
	...
	fin >> index;
	assert(index != -1);
	m_symbol = library.getSymbol(index);
...
void MorphingSprite::storeToTextFile(std::ofstream& fout, const LibraryPanel& library) const
{
