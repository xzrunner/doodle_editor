...
void LayersMgr::loadFromTextFile(std::ifstream& fin, const LibraryPanel& library)
{
	clear();
	...
	for (size_t i = 0; i < size; ++i)
	{
		...
		layer->loadFromTextFile(fin, library);
		m_layers.push_back(layer);
	}
}

void LayersMgr::storeToTextFile(std::ofstream& fout, const LibraryPanel& library) const
{
	fout << "layer " << m_layers.size() << '\n';
	for (size_t i = 0, n = m_layers.size(); i < n; ++i)
		m_layers[i]->storeToTextFile(fout, library);
}
...
void LayersMgr::removeSprite(f2Sprite* sprite)
{
	for (size_t i = 0, n = m_layers.size(); i < n; ++i)
	{
