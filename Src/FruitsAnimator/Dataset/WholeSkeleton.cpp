...
void WholeSkeleton::traverse(IVisitor& visitor)
{
...
void WholeSkeleton::getBounding(AbstractBV& bounding)
{
	traverse(ComputeBoundingVisitor(bounding));
}

WholeSkeleton::Sprite* WholeSkeleton::queryByPos(const f2Vec2& pos)
{
...
WholeSkeleton::Node::Node(const f2Vec2& p) 
	: pos(p) 
{}
...
void WholeSkeleton::Body::onDraw() const
{
	for (size_t i = 0, n = m_nodes.size(); i < n; ++i)
	{
		const f2Vec2& pos = m_nodes[i]->pos;
		wxgui::Render::drawSquare(pos, NODE_RADIUS * 0.5f, f2Colorf(1.0f, 0.0f, 0.0f));
		wxgui::Render::drawRing(pos, NODE_RADIUS, f2Colorf(0.0f, 1.0f, 0.0f));
	}
}

WholeSkeleton::Node* WholeSkeleton::Body::queryNodeByPos(const f2Vec2& pos)
{
	...
	for (size_t i = 0, n = m_nodes.size(); i < n; ++i)
	{
		float dis = f2Math::getDistance(pos, m_nodes[i]->pos);
		if (dis < nearestDis) 
		{
			nearest = m_nodes[i];
			nearestDis = dis;
...
void WholeSkeleton::Body::insertNode(const f2Vec2& pos)
{
...
WholeSkeleton::Sprite::Sprite(Body* body, const f2Vec2& pos)
{
	m_pos = pos;
	m_body = body;
	m_parent = NULL;
...
WholeSkeleton::Sprite::Sprite(const WholeSkeleton::Sprite& sprite, WholeSkeleton::Sprite* parent/* = NULL*/)
	: f2Sprite(sprite), m_relativeCoords(sprite.m_relativeCoords)
{
...
void WholeSkeleton::Sprite::traverse(IVisitor& visitor)
{
...
// 	drawBone(this, this, NULL, times);
 	...
 	glPushMatrix();
 	glTranslatef(m_pos.x, m_pos.y, 1.0f);
 	glRotatef(m_angle * TRANS_RAD_TO_DEG, 0, 0, 1);
 	wxgui::Render::drawSpriteCenter(this);
 	m_body->onDraw();
 	glPopMatrix();
}
...
f2Vec2 WholeSkeleton::Sprite::getNodeWorldCoords(Node* node) const
{
	return m_pos + f2Math::rotateVector(node->pos, m_angle);
}
...
void WholeSkeleton::Sprite::drawBone(const Sprite* bone, const Sprite* curr, const Sprite* currChild, int& times) const
{
	if (curr->m_parent)
	{
		...
		if (bone == curr)
		{
			...
			f2Vec2 offset = f2Math::rotateVector(curr->m_relativeCoords.from->pos, curr->m_relativeCoords.delta);
			glTranslatef(-offset.x, -offset.y, 1.0f);

			glRotatef(curr->m_relativeCoords.delta * TRANS_RAD_TO_DEG, 0, 0, 1);
			wxgui::Render::drawSpriteCenter(bone);
			bone->m_body->onDraw();
			glPopMatrix();
		...
		else
		{
			++times;
			glPushMatrix();
			f2Vec2 offset = f2Math::rotateVector(currChild->m_relativeCoords.to->pos - curr->m_relativeCoords.from->pos, curr->m_relativeCoords.delta);
			glTranslatef(offset.x, offset.y, 1.0f);
			glRotatef(curr->m_relativeCoords.delta * TRANS_RAD_TO_DEG, 0, 0, 1);
		}
	}
	else
	{
		...
		{
			glPushMatrix();
			glTranslatef(bone->getPosition().x, bone->getPosition().y, 1.0f);
			glRotatef(bone->getAngle() * TRANS_RAD_TO_DEG, 0, 0, 1);
			wxgui::Render::drawSpriteCenter(bone);
			bone->m_body->onDraw();
			glPopMatrix();
		...
		else
		{
			++times;
			glPushMatrix();
			f2Vec2 offset = curr->getPosition() + 
				f2Math::rotateVector(currChild->m_relativeCoords.to->pos, curr->getAngle());
			glTranslatef(offset.x, offset.y, 1.0f);
			glRotatef(curr->getAngle() * TRANS_RAD_TO_DEG, 0, 0, 1);
		}
	}
}
...
visit(IObject* object, bool& bFetchNext)
{
	...
	if (bone->m_parent)
	{
		...
		bone->translate(
			...
			+ f2Math::rotateVector(bone->m_relativeCoords.to->pos, bone->m_parent->getAngle())
			- f2Math::rotateVector(bone->m_relativeCoords.from->pos, bone->getAngle())
			);
	}
...
void WholeSkeleton::FetchAllBonesVisitor::visit(IObject* object, bool& bFetchNext)
{
	WholeSkeleton::Sprite* bone = static_cast<WholeSkeleton::Sprite*>(object);
...
void WholeSkeleton::DrawVisitor::visit(IObject* object, bool& bFetchNext)
{
	WholeSkeleton::Sprite* bone = static_cast<WholeSkeleton::Sprite*>(object);
	bone->onDraw();
	bFetchNext = true;
...
WholeSkeleton::ComputeBoundingVisitor::ComputeBoundingVisitor(AbstractBV& bounding)
	: m_bounding(bounding)
{
}

void WholeSkeleton::ComputeBoundingVisitor::visit(IObject* object, bool& bFetchNext)
{
...
PointQueryVisitor(const f2Vec2& pos, WholeSkeleton::Sprite** pSelected)
	: m_pos(pos)
{
