...
void Layer::loadFromTextFile(std::ifstream& fin, const LibraryPanel& library)
{
	clear();
	...
	for (size_t i = 0; i < size; ++i)
	{
		...
		frame->loadFromTextFile(fin, library);
		m_frames.insert(std::make_pair(time, frame));
	}
}

void Layer::storeToTextFile(std::ofstream& fout, const LibraryPanel& library) const
{
	fout << m_name << '\n';
	...
	{
		fout << itr->first << " ";
		itr->second->storeToTextFile(fout, library);
		fout << '\n';
	}
}
...
bool Layer::removeSprite(f2Sprite* sprite)
{
