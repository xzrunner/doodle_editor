...
DelaunayTriangulation::DelaunayTriangulation(const std::vector<f2Vec2>& src, bool bFixBound /*= true*/)
{
	if (src.size() >= 3)
	{
		initSurrondTri(src);
...
void DelaunayTriangulation::getAllTrisInRegion(std::vector<std::vector<f2Vec2> >& triBounds, 
											   const std::vector<f2Vec2>& region) const
{
	...
	for (size_t i = 0, n = tris.size(); i < n; ++i)
	{
		...
		f2Vec2 p = f2Math::getTriGravityCenter(bound[0], bound[1], bound[2]);
		if (f2Math::isPointInArea(p, region))
			triBounds.push_back(bound);
	}
}

void DelaunayTriangulation::getAllTrisInRegion(std::vector<f2Vec2>& triBounds, const std::vector<f2Vec2>& region) const
{
	std::vector<Triangle*> tris = getAllTris();
	for (size_t i = 0, n = tris.size(); i < n; ++i)
	{
...
bool DelaunayTriangulation::insertNode(const f2Vec2& p)
{
	int type = f2Math::checkPosInTriangle(p, m_rootTri->m_nodes[0]->m_pos, m_rootTri->m_nodes[1]->m_pos, m_rootTri->m_nodes[2]->m_pos);
	if (type == -1 || type == 3)
		return false;
...
void DelaunayTriangulation::deleteNodes(const std::vector<f2Vec2>& pos)
{
...
bool DelaunayTriangulation::isBoundNode(const f2Vec2& n) const
{
	Node tmp(n);
...
void DelaunayTriangulation::getBoundLinePos(std::vector<std::vector<f2Vec2> >& bounds) const
{
	std::set<f2Vec2, PosDownCmp> leftPos;
	...
	while (!leftPos.empty())
	{
		...
		while (true)
		{
			...
			for (size_t i = 0; i < (*ptrCurrNode)->m_edges.size(); ++i)
			{
				...
				if (e->isMargin())
				{
					f2Vec2 otherPos(e->m_nStart == *ptrCurrNode ? e->m_nEnd->m_pos : e->m_nStart->m_pos);
					...
					float angle;
					if (bound.size() < 2)
						angle = f2Math::getLineAngle(bound[bound.size() - 1], otherPos);
					else
						angle = f2Math::getAngle(bound[bound.size() - 1], bound[bound.size() - 2], otherPos);
					if (angle < minAngle)
					{
						minAngle = angle;
						nextPos = otherPos;
...
void DelaunayTriangulation::getSurroundPos(const f2Vec2& pos, std::vector<f2Vec2>& surround) const
{
	Node tmp(pos);
	std::set<Node*, NodeCmp>::const_iterator itr = m_nodes.find(&tmp);
	if (itr != m_nodes.end())
	...
	legalizeEdge(bool checkRightTri)
{
	...
	f2Vec2 checkPos(m_tLeft.second->m_pos), fixPos(m_tRight.second->m_pos);
	if (checkRightTri)
		std::swap(checkPos, fixPos);

	f2Vec2 center;
	f2Math::getCircumcenter(m_nStart->m_pos, m_nEnd->m_pos, fixPos, &center);
	if (f2Math::getDistance(checkPos, center) < f2Math::getDistance(fixPos, center))
	{
...
{
	return f2Math::getDistance(m_nStart->m_pos, m_nEnd->m_pos);
}
...
bool DelaunayTriangulation::insertNode(Node* n)
{
	...
	while (true)
	{
		if (curr->m_bLeaf)
		{
			int type = f2Math::checkPosInTriangle(n->m_pos, curr->m_nodes[0]->m_pos, curr->m_nodes[1]->m_pos, curr->m_nodes[2]->m_pos);
			switch (type)
			{
			case 0:
		...
		else
		{
			for (size_t i = 0; i < curr->m_children.size(); ++i)
			{
				...
				int type = f2Math::checkPosInTriangle(n->m_pos, child->m_nodes[0]->m_pos, child->m_nodes[1]->m_pos, child->m_nodes[2]->m_pos);
				if (type != -1)
				{
					curr = child;
					break;
...
void DelaunayTriangulation::initSurrondTri(const std::vector<f2Vec2>& src)
{
	f2AABB scope;
	f2Math::getMBR(src, &scope);
	float extend = std::max(scope.xLength(), scope.yLength()) * BOUND_EXTEND_TIMES;
	f2Vec2 center(0.5f * (scope.xMin + scope.xMax), 0.5f * (scope.yMin + scope.yMax));

	f2Vec2 p0(center.x, center.y + extend), 
...
void DelaunayTriangulation::initSrcNodes(const std::vector<f2Vec2>& src)
{
	...
	for ( ; itr != m_nodes.end(); ++itr)
	{
		...
		if (bSingle)
		{
			...
			for (size_t i = 0; i < (*itr)->m_edges.size(); ++i)
			{
				...
				{
					f2Vec2 other = e->leftNode()->m_pos;
					float maxLenEdge = std::max(
						f2Math::getDistance(e->m_nStart->m_pos, other),
						f2Math::getDistance(e->m_nEnd->m_pos, other)
						);
				...
				if (!tri->isFake() && tri->m_bRemoved)
				{
					f2Vec2 other = e->rightNode()->m_pos;
					float maxLenEdge = std::max(
						f2Math::getDistance(e->m_nStart->m_pos, other),
						f2Math::getDistance(e->m_nEnd->m_pos, other)
						);
