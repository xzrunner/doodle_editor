#ifndef _F2_DELAUNAY_TRIANGULATION_H_
#define _F2_DELAUNAY_TRIANGULATION_H_
...
namespace F2D
{
	...
	public:
		DelaunayTriangulation(const std::vector<f2Vec2>& src, bool bFixBound = true);
		~DelaunayTriangulation();

		const std::vector<Triangle*>& getAllTris() const { return m_tris; }
		const std::set<Node*, NodeCmp>& getAllNodes() const { return m_nodes; }
		void getAllTrisInRegion(std::vector<std::vector<f2Vec2> >& triBounds, const std::vector<f2Vec2>& region) const;
		void getAllTrisInRegion(std::vector<f2Vec2>& triBounds, const std::vector<f2Vec2>& region) const;

		bool insertNode(const f2Vec2& p);
		void deleteNodes(const std::vector<f2Vec2>& pos);

		bool isBoundNode(const f2Vec2& n) const;
		void getBoundLinePos(std::vector<std::vector<f2Vec2> >& bounds) const;

		void getSurroundPos(const f2Vec2& pos, std::vector<f2Vec2>& surround) const;
	...
	public:
		...
		{
		public:
			Node(const f2Vec2& pos, bool bFake = false) 
		...
		public:
			...
			void getNodesPos(std::vector<f2Vec2>& pos) const {
	...
	private:
		...
		void initSurrondTri(const std::vector<f2Vec2>& src);
		void initSrcNodes(const std::vector<f2Vec2>& src);
	...
	public:
		...
		{
		public:
			bool operator () (const f2Vec2& lhs, const f2Vec2&  rhs) const {
				return lhs.y > rhs.y
...
#endif // _F2_DELAUNAY_TRIANGULATION_H_
