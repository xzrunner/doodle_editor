...
{
	std::vector<BodyData*> bodies;
	stage->traverseBodies(FetchAllVisitor<BodyData>(bodies));

	std::vector<JointData*> joints;
	stage->traverseJoints(FetchAllVisitor<JointData>(joints));
	...
	case JointData::e_wheelJoint:
		{
			...
			float len = f2Math::getDistance(wJoint->localAxisA, f2Vec2());
