					   ...
					   wxWindow*& stage, wxWindow*& toolbar)
{
	...
	property = m_property= new wxgui::PropertySettingPanel(leftHorizontalSplitter);

	stage = m_stage = new StagePanel(leftVerticalSplitter, m_libraryPanel);

	toolbar = new ToolbarPanel(rightVerticalSplitter, m_stage, m_libraryPanel, m_property);
}