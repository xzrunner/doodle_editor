...
{
	const f2Vec2& center = m_batch.getCenter();
...
void PasteSpriteOP::SpriteBatch::
insertToSpritesImpl(SpritesPanelImpl* spritesImpl, const f2Vec2& pos,
					bool isHorMirror, bool isVerMirror)
{
	for (size_t i = 0, n = m_selected.size(); i < n; ++i)
	{
		ISprite* sprite = m_selected[i];
		f2Vec2 fixed = sprite->getPosition() - m_center;
		if (isHorMirror)
...
void PasteSpriteOP::SpriteBatch::
draw(const f2Vec2& pos, bool isHorMirror, bool isVerMirror) const
{
	if (!m_selected.empty() && pos.isValid())
	{
		...
		for (size_t i = 0, n = m_selected.size(); i < n; ++i)
		{
			const f2Vec2& pos = m_selected[i]->getPosition();
