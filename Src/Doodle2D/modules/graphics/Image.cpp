...
#ifdef D2D_WINDOWS
Image::Image(const std::string& pPath)
	: m_type(e_unknown)
	, m_resource(pPath)
{
	clear();
...
#elif defined D2D_ANDROID_NATIVE
Image::Image(android_app* pApplication, const std::string& pPath)
	: m_type(e_unknown)
...
uint8_t* Image::loadImage()
{
#ifdef LOAD_BY_SOIL
	return loadBySOIL();
#else
	std::string filepath = m_resource.getPath();
	...
	else if (filepath.find(".jpg") || filepath.find(".jpeg"))
	{
		m_type = e_jpg;
		return loadByLibJpeg();
	}
	else
	{
		m_type = e_unknown;
...
uint8_t* Image::loadBySOIL()
{
 	int channels;
 	return SOIL_load_image(
...
uint8_t* Image::loadByLibPng()
{
	...
	if (m_resource.read(lHeader, sizeof(lHeader)) != STATUS_OK)
		goto ERROR;
	...
	png_set_read_fn(lPngPtr, &m_resource, callback_read);
	...
	m_width = lWidth; m_height = lHeight;
...
//   jpeg_stdio_src(&cinfo, infile);
   void* data = const_cast<void*>(m_resource.bufferize());
   jpeg_mem_src(&cinfo, data, m_resource.getLength());
   ...
   m_format = GL_RGB;
   unsigned char* pixels = new unsigned char[m_width * m_height * channels];
...
//////////////////////////////////////////////////////////////////////////
// another way to load jpeg from memory
//////////////////////////////////////////////////////////////////////////
// use GNU's fmemopen() function which should be declared in stdio.h header file.
