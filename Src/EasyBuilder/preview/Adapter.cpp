...
Scene* Adapter::builder2Scene(const ebuilder::Scene& scene)
{
	...
	const std::vector<ebuilder::Actor*>& actors = scene.getActors();
	for (size_t i = 0, n = actors.size(); i < n; ++i)
	{
		ebuilder::Actor* actor = actors[i];
...
Behavior* Adapter::builder2Behavior(wxgui::ISprite* sprite, const ebuilder::Behavior& behavior)
{
