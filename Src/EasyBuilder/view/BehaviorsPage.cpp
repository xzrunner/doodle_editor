...
BehaviorsPage::BehaviorsPage(wxWindow* parent)
	: wxgui::ILibraryPage(parent, wxT("Behaviors"))
{
	initLayout();
	initBehaviors();
}

bool BehaviorsPage::isHandleSymbol(wxgui::ISymbol* symbol) const
{
	return false;
}
...
void BehaviorsPage::initLayout(bool draggable /*= true*/)
{
	...
	if (!m_list)
		m_list = new wxgui::LibraryList(this, wxT("behaviors"), /*false*/true);
