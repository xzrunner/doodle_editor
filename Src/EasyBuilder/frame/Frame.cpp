...
void Frame::onNew(wxCommandEvent& event)
{
	setCurrFilename();
	clear();
	Context::Instance()->library->getScenePage()->insertScene();
}
...
void Frame::onOpen(wxCommandEvent& event)
{
	...
	if (dlg.ShowModal() == wxID_OK)
	{
		...
		wxgui::Settings::RESOURCE_PATH.insert(wxgui::FilenameTools::getFileDir(dlg.GetPath()) + "\\");
...
void Frame::onSaveAs(wxCommandEvent& event)
{
	...
	if (dlg.ShowModal() == wxID_OK)
	{
		m_currFilename = wxgui::FilenameTools::getFilePathExceptExtension(dlg.GetPath()) + wxT(".json");
		FileIO::store(m_currFilename);
	}
}
...
void Frame::onPreview(wxCommandEvent& event)
{
	...
	preview::Dialog dlg(this, context->stage->getDesignerPage()->getScene(), 
		context->library);
	dlg.ShowModal();
...
wxMenu* Frame::initSettingsBar()
{
	...
	menu->Append(myID_MENU_SETTINGS_OPERATING, wxT("Operation"));
	Connect(myID_MENU_SETTINGS_OPERATING, wxEVT_COMMAND_MENU_SELECTED, 
...
void Frame::initWorkingFrame()
{
	...
	context->stage = new StagePanel(m_hRightSplitter);
