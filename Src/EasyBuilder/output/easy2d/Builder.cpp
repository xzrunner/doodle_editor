...
#include "output/StyleCPlusPlus.h"
...
void Builder::buildActor(const Actor& actor, SceneContent& content) const
{
	...
	std::string name = actor.m_name.ToStdString();
	std::string filename = "\""+wxgui::FilenameTools::getFilenameWithExtension(actor.getSymbol().getFilepath())+"\"";
	...
	if (actor.getScale() != 1)
	{
		std::string scale = wxString::FromDouble(actor.getScale(), 2) + "f";
	...
	case e_ChangeImage:
		{
			...
			std::string name = "\""+wxgui::FilenameTools::getFilenameWithExtension(path)+"\"";
