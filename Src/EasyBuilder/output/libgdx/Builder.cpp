	...
	void Builder::buildScenePage(const SceneItem& scene, Page* page)
	{
		...
		}
		{
			...
			for (size_t i = 0, n = actors.size(); i < n; ++i)
			{
				...
				wxString name = wxgui::FilenameTools::getFilename(path);
				wxString filename = wxgui::FilenameTools::getFilenameWithExtension(path);
			...
			gen.line();
			for (size_t i = 0, n = actors.size(); i < n; ++i)
				java::declare(gen, "Image", actors[i]->m_name.ToStdString());
			gen.line();
			{
...
//					gen.line();
	...
	void Builder::buildActor(const Actor& actor, SceneContent& content)
	{
		...
		std::string tex_name = "r_"+wxgui::FilenameTools::getFilename(path);
		...
		std::string _ox = wxString::FromDouble(actor.getSymbol().getWidth()*0.5f, 1) + "f";
		std::string _oy = wxString::FromDouble(actor.getSymbol().getHeight()*0.5f, 1) + "f";

		std::string _angle = wxString::FromDouble(actor.getAngle()*180/PI, 2) + "f";
		...
		case e_ChangeImage:
			{
				...
				std::string tex_region_name = "r_"+wxgui::FilenameTools::getFilename(path);
		...
		case e_Rotate:
			{
				...
				std::string end = wxString::FromDouble(rotate->end*180/PI, 1) + "f";
	...
	{
		const float px = actor.getPosition().x + Game::WIDTH * 0.5f - actor.getSymbol().getWidth() * 0.5f,
			py = actor.getPosition().y + Game::HEIGHT * 0.5f - actor.getSymbol().getHeight() * 0.5f;
		x = wxString::FromDouble(px, 1) + "f";
		y = wxString::FromDouble(py, 1) + "f";
	}

	void Builder::buildPosition(const f2Vec2& pos, std::string& x, std::string& y)
	{
