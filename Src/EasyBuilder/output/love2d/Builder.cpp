...
namespace love2d
{
	...
	//		lua::TableAssign tButtons(*content.genButtons, "self.buttons");
			...
			gen.line();
			const std::vector<Actor*>& actors = scene.getScene()->getActors();
	...
	void Builder::buildActor(const Actor& actor, SceneContent& content)
	{
		...
		std::string sw = wxString::FromDouble(actor.getSymbol().getWidth(), 1);
		std::string sh = wxString::FromDouble(actor.getSymbol().getHeight(), 1);
		...
		if (isBtn)
		{
			...
			// self.buttons["actor"] = Button(image, x, y, angle, sx, sy, sw, sh)
			lua::assign(*content.genButtons, "self.buttons[\""+name+"\"]", new_btn);
			for (size_t i = 0, n = behaviors.size(); i < n; ++i)
			{
				if (behaviors[i]->type() == e_MouseButton)
				{
					...
					for (size_t j = 0, m = mouseButton->satisfies.size(); j < m; ++j)
						buildBehavior("self.buttons."+name, func, mouseButton->satisfies[j], content);
				}
				else
				{
					buildBehavior("self.buttons."+name, "addAction", behaviors[i], content);
				}
			}
		}
		else
		{
			...
			std::string new_actor = lua::call("", "Actor", 8, image.c_str(), x.c_str(), y.c_str(), 
				angle.c_str(), sx.c_str(), sy.c_str(), sw.c_str(), sh.c_str());
			// self.actors["actor"] = Actor(image, x, y, angle, sx, sy, sw, sh)
			lua::assign(*content.genActors, "self.actors[\""+name+"\"]", new_actor);
			// behaviors
			for (size_t i = 0, n = behaviors.size(); i < n; ++i)
				buildBehavior("self.actors."+name, "addAction", behaviors[i], content);
		}
	}
		...
		case e_Move:
			{
				...
				const float duration = f2Math::getDistance(move->start, move->end) / move->speed;
		...
		case e_Rotate:
			{
				...
				std::string start = wxString::FromDouble(rotate->start*180/PI, 1);
				std::string end = wxString::FromDouble(rotate->end*180/PI, 1);
	...
	{
		std::string filename = "\""+wxgui::FilenameTools::getFilenameWithExtension(filepath)+"\"";

		// graphics[filename]
		std::string image = "graphics["+filename+"]";
		// if not graphics[filename] then
		if (!content.genImages->existLine("if not "+image+" then"))
		{
			lua::If _if(*content.genImages, "not "+image);

			// love.graphics.newImage(filename)
			std::string new_image = lua::call("", "love.graphics.newImage", 1, filename.c_str());
			// graphics[filename] = love.graphics.newImage(filename)
			lua::assign(*content.genImages, image, new_image);
		}

		return image;
	}

	void Builder::transCoords(const f2Vec2& pos, std::string& x, std::string& y)
	{
